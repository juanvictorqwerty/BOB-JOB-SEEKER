...
// "roles" is a reserved word in some SQL dialects — always quote it with an explicit @Table to be safe.
@Table(name = "roles")
@Data
@NoArgsConstructor
@AllArgsConstructor
public class Roles {
    ...
    // Numeric rank/privilege level for this role — higher number = higher authority.
    // Used for hierarchical permission checks (e.g. can RECRUITER perform this action?).
    // Unique: no two roles can share the same numeric rank, enforcing a strict hierarchy.
