...
@Table(name = "job_posts", indexes = {
        ...
        // Index on title: supports the unique constraint below and prefix/exact search queries
...
public class JobPost {
    ...
    // @UpdateTimestamp auto-populates on every Hibernate UPDATE — tracks last edit timestamp
    @UpdateTimestamp
    ...
    //
    // CRITICAL FIX: The field type MUST be the target entity (Company), NOT a raw UUID.
    // The previous code had @Column + @ManyToOne + @JoinColumn all on a 'private UUID companyId' field.
    ...
    // updatable=false: once a job post is assigned to a company, that cannot change.
    @ManyToOne(fetch = FetchType.LAZY) // LAZY: only load company data when explicitly accessed
    @JoinColumn(name = "company_id", nullable = false, updatable = false)
    private Company company;

    // Diagram marks title as (U) — enforcing uniqueness at DB level prevents duplicate job listings
    @Column(name = "title", nullable = false, unique = true)
    private String title;

    // JSONB: flexible schema for description (e.g. {text, requirements, benefits, experience_level})
    ...
    // Double/Float use binary floating-point which accumulates rounding errors for monetary values
    // e.g., 99.99 stored as Double may become 99.98999999999... in the DB.
    // BigDecimal maps to NUMERIC(15,2) in PostgreSQL — exact decimal arithmetic, no precision loss.
    @Column(name = "salary", nullable = true, precision = 15, scale = 2)
    private BigDecimal salary;

    // JSONB: flexible location schema (e.g. {city, country, lat, lon, remote: true})
    ...
    // Essential for filtering open vs. closed job postings — the most fundamental business query on this table.
    // Boolean WRAPPER (not primitive) for forward-compatibility if nullable state is ever needed.
