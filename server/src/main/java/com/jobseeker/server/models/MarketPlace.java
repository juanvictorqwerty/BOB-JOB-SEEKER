...
@Table(name = "marketplace", indexes = {
        ...
        // Note: for deep JSONB sub-field queries, consider a GIN index in a Flyway
        // migration
...
public class MarketPlace {
    ...
    // @UpdateTimestamp auto-populates on every Hibernate UPDATE — tracks listing
    // edits
    @UpdateTimestamp
    ...
    // UUID.
    // The previous code had @Column + @ManyToOne + @JoinColumn on a 'private UUID
    // postingUserId'.
    ...
    //
    // ALSO FIXED: @JoinColumn name was "user_id" — incorrect, would shadow the FKs
    // in other tables.
    // Changed to "posting_user_id" to uniquely identify this FK in the marketplace
    // table.
    //
    ...
    // Indexed above for search performance — title is the primary lookup field for
    // marketplace listings
    @Column(name = "title", nullable = false)
    private String title;

    // JSONB: flexible description schema — e.g. {"text": "...", "tags":
    // ["electronics", "used"]}
    ...
    // e.g. ["marketplace/{user-id}/{listing-id}/img1.jpg",
    // "marketplace/{user-id}/{listing-id}/img2.jpg"]
