...
@Table(name = "replies", indexes = {
        // Index on company_id: fast lookup of all replies directed at a specific company
        @jakarta.persistence.Index(name = "idx_replies_company_id", columnList = "company_id"),
        // Index on user_id: fast lookup of all replies submitted by a specific user (applicant)
        @jakarta.persistence.Index(name = "idx_replies_user_id", columnList = "user_id")
})
// @Data generates getters, setters, toString, equals, hashCode — required for any service-layer access
...
public class Replies {
    ...
    @Id
    // UUID auto-generated on insert — changed from String to UUID to match diagram design
    ...
    // @UpdateTimestamp auto-populates on every Hibernate UPDATE — tracks when the reply status changes
    @UpdateTimestamp
    ...
    //
    // CRITICAL FIX: The field type MUST be the target entity (Company), NOT a raw UUID or String.
    ...
    // combining @Column + @JoinColumn on the same field causes a column duplication conflict.
    @ManyToOne(fetch = FetchType.LAZY) // LAZY: company data only loaded when explicitly accessed — prevents N+1
    @JoinColumn(name = "company_id", nullable = false, updatable = false)
    private Company company;

    // @ManyToOne: many replies can be submitted by one user (the job applicant / responder).
    // Same fix as above — field must be Users entity, not a raw String or UUID.
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "user_id", nullable = false, updatable = false)
    private Users user;

    // Stores the MinIO object key or URL pointing to the applicant's submitted file (CV, cover letter, etc.)
    // length=1024 because MinIO object keys easily exceed the default 255-char column length
    // e.g.: "resumes/{user-uuid}/{timestamp}-{original-filename}.pdf"
    @Column(name = "file_url", nullable = false, length = 1024)
    private String fileUrl;

    // Boolean WRAPPER (not primitive) — required because this column has 3 valid states:
    ...
    //
    // Renamed from isAccepted → isApproved to match the diagram field name "isApproved"
