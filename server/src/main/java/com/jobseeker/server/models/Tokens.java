...
@Table(
    name = "tokens",
    indexes = {
        // Index on value: token lookup happens on EVERY authenticated request — must be extremely fast
...
public class Tokens {
    ...
    // @ManyToOne: many tokens can belong to one user (one user = multiple devices/sessions).
    //
    // CRITICAL FIX: The field type MUST be the target entity (Users), NOT a raw UUID.
    // The previous code had @Column + @ManyToOne + @JoinColumn all on a 'private UUID userId'.
    ...
    // length=512: full JWT strings regularly exceed the default 255-char column limit.
    ...
    // Instant in Java is always UTC by definition; no DB-level timezone hint is needed here.
    @UpdateTimestamp
    @Column(name = "updated_at")
    private Instant updatedAt;

    // The point in time when this token expires and must be rejected by the auth filter.
    // Must be explicitly set by the service layer on token creation (e.g. now + 7 days).
    // FIXED: removed 'columnDefinition = "UTC"' — same reason as above; Instant is already UTC.
    ...
    // Using Boolean wrapper instead of boolean primitive avoids accidental null-pointer issues
    // and keeps the field consistent with nullable Boolean fields elsewhere in the schema.
